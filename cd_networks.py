...
class AdaINGen(nn.Module):
    ...
    def __init__(self, input_dim, seg_dim, params):
        ...
        self.dec = StyleBasedDecoder(n_downsample, n_res, self.enc_content.output_dim, input_dim, res_norm='adain', activ=activ, pad_type=pad_type)
        
        # MLP to generate AdaIN parameters
        self.mlp = MLP(style_dim, self.get_num_adain_params(self.dec), mlp_dim, 3, norm='none', activ=activ)

        self.enc_seg = ContentEncoder(n_downsample, n_res, input_dim, dim // 2, 'in', activ, pad_type=pad_type)
    ...
    def forward(self, images):
        ...
        content, style_fake, seg_content = self.encode(images)
        images_recon = self.decode(content, style_fake, seg_content)
        return images_recon
    ...
    def encode(self, images):
        ...
        seg_content = self.enc_seg(images)
        return content, style_fake, seg_content

    def decode(self, content, style, seg_content):
        adain_params = self.mlp(style)
        self.assign_adain_params(adain_params, self.dec)
        feat_final, intermed = self.dec(seg_content, content)
        return feat_final
...
class StyleBasedDecoder(nn.Module):
    def __init__(self, n_upsample, n_res, dim, output_dim, res_norm='adain', activ='relu', pad_type='zero'):
        ...
        self.model += [StyleBasedBlocks(n_res, dim, res_norm, activ, pad_type=pad_type)]
        # upsampling blocks
    ...
    def forward(self, final_feats, intermed):
        return self.model(final_feats, intermed)
...
class StyleBasedBlocks(nn.Module):
    def __init__(self, num_blocks, dim, norm='adain', activation='relu', pad_type='zero'):
        ...
        for i in range(num_blocks):
            self.model += [StyleBasedBlock(dim, norm=norm, activation=activation, pad_type=pad_type)]
        self.model = MultInputSequential(*self.model)
...
class UpsampleBlock(nn.Module):
    ...
    def forward(self, final_feats, intermed):
        return self.upsample(final_feats), self.upsample(intermed)
...
class StyleBasedBlock(nn.Module):
    def __init__(self, dim, norm='adain', activation='relu', pad_type='zero'):
        ...
        self.final_model = ResBlock(dim, norm, activation, pad_type)
        self.intermed_model = ResBlock(dim, norm, activation, pad_type)

    def forward(self, final_feats, intermed):
        intermed = self.intermed_model(intermed)
        return intermed + self.final_model(final_feats), intermed 
...
class Conv2dWrapper(nn.Module):
    ...
    def forward(self, final_feats, intermed):
        return self.conv_block(final_feats), self.conv_block(intermed)
