...
from secunit_networks import AdaINGen, MsImageDis, VAEGen
from secunit_utils import weights_init, get_model_list, vgg_preprocess, load_vgg16, get_scheduler
from torch.autograd import Variable
import torch
import torch.nn as nn
import os

class SECUNIT_Trainer(nn.Module):
    def __init__(self, hyperparameters):
        super(SECUNIT_Trainer, self).__init__()
        ...
        if hyperparameters.get("edge_loss_w", -1) == -1:
            print("Not using segmentation edge loss")
            hyperparameters["edge_loss_w"] = 0
    ...
    #def forward(self, x_a, x_a_seg, x_b, x_b_seg):
        #self.eval()
        #s_a = Variable(self.s_a)
        #s_b = Variable(self.s_b)
        #c_a, s_a_fake, c_a_seg = self.gen_a.encode(x_a, x_a_seg)
        #c_b, s_b_fake, c_b_seg = self.gen_b.encode(x_b, x_b_seg)
    ...
    def gen_update(self, x_a, real_a_seg, x_b, real_b_seg, hyperparameters):
        ...
        # encode
        c_a, s_a_prime, c_a_seg = self.gen_a.encode(x_a, real_a_seg)
        c_b, s_b_prime, c_b_seg = self.gen_b.encode(x_b, real_b_seg)
        # segmentations
        x_a_seg = self.gen_a.dec_seg(c_a_seg)
        x_b_seg = self.gen_b.dec_seg(c_b_seg)        
        
        # decode (within domain)
        x_a_recon = self.gen_a.decode(c_a, s_a_prime, c_a_seg, real_a_seg) # teacher forcing, could use recon seg_img
        x_b_recon = self.gen_b.decode(c_b, s_b_prime, c_b_seg, real_b_seg) # 
        # decode (cross domain)
        ...
        x_ba = self.gen_a.decode(c_ba, s_a, c_ba_seg, x_ba_seg) #was c_b
        x_ab = self.gen_b.decode(c_ab, s_b, c_ab_seg, x_ab_seg) #was c_a

        # encode again
        c_ba_recon, s_a_recon, c_ba_seg_recon = self.gen_a.encode(x_ba, x_ba_seg) #was c_b_recon
        c_ab_recon, s_b_recon, c_ab_seg_recon = self.gen_b.encode(x_ab, x_ab_seg) #was c_a_recon
        ...
        x_aba = self.gen_a.decode(c_a_recon, s_a_prime, c_a_seg_recon, real_a_seg) if hyperparameters['recon_x_cyc_w'] > 0 else None
        x_bab = self.gen_b.decode(c_b_recon, s_b_prime, c_b_seg_recon, real_b_seg) if hyperparameters['recon_x_cyc_w'] > 0 else None
        ...
        self.loss_gen_adv_seg_a = self.dis_seg_a.calc_gen_loss(x_ba_seg) # + self.dis_seg_a.calc_gen_loss(x_a_seg)
        self.loss_gen_adv_seg_b = self.dis_seg_b.calc_gen_loss(x_ab_seg) # + self.dis_seg_b.calc_gen_loss(x_b_seg)

        # domain-invariant perceptual loss
        self.loss_gen_vgg_a = self.compute_vgg_loss(self.vgg, x_ba, x_b) if hyperparameters['vgg_w'] > 0 else 0
        self.loss_gen_vgg_b = self.compute_vgg_loss(self.vgg, x_ab, x_a) if hyperparameters['vgg_w'] > 0 else 0

        # Segmentation edge loss
        self.loss_edge_a = self.compute_edge_loss(x_ab_seg, self.gen_b.n_seg_classes, real_a_seg, self.gen_a.n_seg_classes) if hyperparameters['edge_loss_w'] > 0 else 0
        self.loss_edge_b = self.compute_edge_loss(x_ba_seg, self.gen_a.n_seg_classes, real_b_seg, self.gen_b.n_seg_classes) if hyperparameters['edge_loss_w'] > 0 else 0
        ...
        self.loss_gen_total_overflow = hyperparameters["edge_loss_w"] * self.loss_edge_a + hyperparameters["edge_loss_w"] * self.loss_edge_b
        self.loss_gen_total_overflow.backward()
        self.gen_opt.step()

    def make_one_hot(self, labels, nclasses):
        one_hot = torch.cuda.FloatTensor(labels.size(0), nclasses, labels.size(2), labels.size(3)).zero_()
        target = one_hot.scatter_(1, labels.data, 1)
        #target = torch.autograd.Variable(target)
        return target

    def check_edges(self, slice1, slice2):
        edges = slice1 + slice2
        edges -= 1
        edges = torch.abs(torch.abs(edges) - 1)
        
        #edges[edges == 2] = 0
        return edges

    def get_edges(self, t, nclasses):
        if t.size(1) == 1:
            t = self.make_one_hot(t, nclasses)
        edge = Variable(torch.FloatTensor(t.size()).zero_().cuda())
        edge[:,:,:,1:] = torch.clamp(self.check_edges(t[:, :, :, 1:], t[:, :, :, :-1]), max=1)
        edge[:,:,:,:-1] = torch.clamp(edge[:,:,:,:-1] + self.check_edges(t[:, :, :, 1:], t[:, :, :, :-1]), max=1)
        edge[:,:,1:,:] = torch.clamp(edge[:,:,1:,:] + self.check_edges(t[:, :, 1:, :], t[:, :, :-1, :]), max=1)
        edge[:,:,:-1,:] = torch.clamp(edge[:,:,:-1,:] + self.check_edges(t[:, :, 1:, :], t[:, :, :-1, :]), max=1)
        
        mask = torch.sum(edge, dim=1, keepdim=True)
    ...
    def compute_edge_loss(self, seg, nclasses1, target, nclasses2):
        seg_edges = self.get_edges(seg, nclasses1)
        t_edges = self.get_edges(target, nclasses2)
        return torch.sum(torch.abs(seg_edges - t_edges))
    ...
    def sample(self, x_a, seg_a, x_b, seg_b, n_random_codes=10):
        x_ab_rand, x_ba_rand = [],[]
        ...
        if len(x_b.size()) == 3:
            x_b = x_b.unsqueeze(0)
        if len(seg_a.size()) == 3:
            seg_a = seg_a.unsqueeze(0)
        if len(seg_b.size()) == 3:
            seg_b = seg_b.unsqueeze(0)
        
        for i in range(x_a.size(0)):
            seg_a_i = seg_a[i].unsqueeze(0)
            seg_b_i = seg_b[i].unsqueeze(0)
            c_a, s_a_fake, c_a_seg = self.gen_a.encode(x_a[i].unsqueeze(0), seg_a_i)
            c_b, s_b_fake, c_b_seg = self.gen_b.encode(x_b[i].unsqueeze(0), seg_b_i)
            x_a_recon.append(self.gen_a.decode(c_a, s_a_fake, c_a_seg, seg_a_i))
            x_b_recon.append(self.gen_b.decode(c_b, s_b_fake, c_b_seg, seg_b_i))
            #x_a_seg.append(self.gen_a.decode_seg(c_a_seg))
            #x_b_seg.append(self.gen_b.decode_seg(c_b_seg))
            
            c_ab, c_ab_seg = self.gen_b.translate(c_a, c_a_seg)
            c_ba, c_ba_seg = self.gen_a.translate(c_b, c_b_seg)
            x_ab_seg, x_ba_seg = self.gen_b.dec_seg(c_ab_seg), self.gen_a.dec_seg(c_ba_seg)
            x_a_seg.append(x_ab_seg)
            x_b_seg.append(x_ba_seg)

            x_ba1.append(self.gen_a.decode(c_ba, s_a1[i].unsqueeze(0), c_ba_seg, x_ba_seg))
            x_ba2.append(self.gen_a.decode(c_ba, s_a2[i].unsqueeze(0), c_ba_seg, x_ba_seg))
            x_ab1.append(self.gen_b.decode(c_ab, s_b1[i].unsqueeze(0), c_ab_seg, x_ab_seg))
            x_ab2.append(self.gen_b.decode(c_ab, s_b2[i].unsqueeze(0), c_ab_seg, x_ab_seg))
            for n in range(n_random_codes):
                x_ab_rand[n].append(self.gen_b.decode(c_ab, s_b_rand[n].unsqueeze(0), c_ab_seg, x_ab_seg))
                x_ba_rand[n].append(self.gen_a.decode(c_ba, s_a_rand[n].unsqueeze(0), c_ba_seg, x_ba_seg))
    ...
    def dis_update(self, x_a, x_a_seg, x_b, x_b_seg, hyperparameters):
        ...
        s_b = Variable(torch.randn(x_b.size(0), self.style_dim, 1, 1).cuda())
        # encode
        c_a, _, c_a_seg = self.gen_a.encode(x_a, x_a_seg)
        c_b, _, c_b_seg = self.gen_b.encode(x_b, x_b_seg)
        # segmentation
        ...
        # decode (cross domain)
        x_ba = self.gen_a.decode(c_ba, s_a, c_ba_seg, fake_ba_seg)
        x_ab = self.gen_b.decode(c_ab, s_b, c_ab_seg, fake_ab_seg)
        # D loss
        self.loss_dis_a = self.dis_a.calc_dis_loss(x_ba.detach(), x_a)
        self.loss_dis_b = self.dis_b.calc_dis_loss(x_ab.detach(), x_b)
        self.loss_dis_seg = self.dis_seg_a.calc_dis_loss(fake_ba_seg.detach(), x_a_seg, True) + self.dis_seg_b.calc_dis_loss(fake_ab_seg.detach(), x_b_seg, True)
            # + self.dis_seg_a.calc_dis_loss(fake_a_seg.detach(), x_a_seg, True) + self.dis_seg_b.calc_dis_loss(fake_b_seg.detach(), x_b_seg, True) +
    ...
    def sample_a(self, x_a, seg_a, n_random_codes=10):
        x_ab_rand, x_s_rand = [], []
        for i in range(n_random_codes):
            x_ab_rand.append([])
            x_s_rand.append([])
        ...
        for i in range(x_a.size(0)):
            ...
            for n in range(n_random_codes):
                x_ab_rand[n].append(self.gen_b.decode(c_ab, s_b_rand[n].unsqueeze(0), c_ab_seg, x_ab_seg))
                x_s_rand[n].append(self.gen_a.decode(c_a, s_a_rand[n].unsqueeze(0), c_a_seg, seg_a_i))
        ...
        self.train()
        return (x_a, x_a_recon, *x_ab_rand), (torch.cat(x_a_seg)), x_s_rand

    def sample_b(self, x_b, seg_b, n_random_codes=10):
        x_ba_rand, x_s_rand = [], []
        for i in range(n_random_codes):
            x_ba_rand.append([])
            x_s_rand.append([])
        ...
        for i in range(x_b.size(0)):
            ...
            for n in range(n_random_codes):
                x_ba_rand[n].append(self.gen_a.decode(c_ba, s_a_rand[n].unsqueeze(0), c_ba_seg, x_ba_seg)) 
                x_s_rand[n].append(self.gen_b.decode(c_b, s_b_rand[n].unsqueeze(0), c_b_seg, seg_b_i))
        ...
        for i in range(n_random_codes):
            ...
            x_s_rand[i] = torch.cat(x_s_rand[i])
        self.train()
        return (x_b, x_b_recon, *x_ba_rand), (torch.cat(x_b_seg)), x_s_rand
